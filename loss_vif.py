...
class fusion_loss_med(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused):
        ...
        std = torch.mean(torch.std(torch.abs(image_fused),dim=[2,3],keepdim=False, unbiased=False))
        fusion_loss = loss_l1 + 1*loss_gradient + loss_SSIM - 50*std
        return fusion_loss

def corr(image_a, image_b):
        image_a = image_a-torch.mean(image_a,dim=[2,3],keepdim=True)
        image_b = image_b-torch.mean(image_a,dim=[2,3],keepdim=True)
        out = torch.sum(image_a*image_b)/(torch.sum(image_a*image_a)*torch.sum(image_b*image_b))
        return out
...
class SCD_Loss(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused):
        a = corr(image_fused-image_A, image_B)
        b = corr(image_fused-image_B, image_A)
        loss = a+b
        return loss
