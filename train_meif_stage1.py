...
# loss
from losses import ssim_loss_ir,ssim_loss_vi
...
def parse_args():
    ...
    parser.add_argument('--ubatch_size', default=3, type=int)
...
def train(train_loader_mef,model,t_model, optimizer,epoch,scheduler=None):
    ...
    for i, (under,over,gt)  in tqdm(enumerate(train_loader_mef), total=len(train_loader_mef)):
        ...
        out_t_y,_,_,_ = t_model(undery,overy)
    
        mef_loss = ssim_loss_vi(outy,gty)
        consis_loss = l_grad(out_t_y.detach(),outy)
        total_loss = 2*mef_loss + 1*consis_loss
    ...
    for i, (under,over,gt)  in tqdm(enumerate(test_loader_ir), total=len(test_loader_ir)):
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                outy,_,_,_ = model(undery,overy)
                out = torch.cat((outy,fcr, fcb),dim=1)
...
def main():
   ...
   # supervised train_data 2
    ...
    model_path = "./models/model/ufnet_mfif_deeper.pth"
    ...
    for epoch in range(args.epochs):
        ...
        model.zero_grad()
        train_log = train(train_loader_mef,model,t_model, optimizer,epoch)

        scheduler_f.step()
        if (epoch+1) % 1 == 0:
            test(scheduler_f, model,t_model, test_loader_ir)
